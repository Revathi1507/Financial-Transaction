...
"T" + (transactions.size() + 1), accountId, "DEPOSIT", amount, new Date().toString(), 
remarks)); 
} 
...
acc.setBalance(acc.getBalance() - amount); 
transactions.add(new TransactionRecord( 
"T" + (transactions.size() + 1), accountId, "WITHDRAW",amount,new Date().toString(), 
remarks )); 
} 
public void transfer(String fromAccountId, String toAccountId, double amount, String 
remarks) 
throws AccountNotFoundException, InvalidTransactionException, 
InsufficientBalanceException { 
if (amount <= 0) { 
...
transactions.add(new TransactionRecord( 
"T" + (transactions.size() + 1), fromAccountId,"TRANSFER",amount,new Date().toString(), 
remarks 
)); 
transactions.add(new TransactionRecord( 
"T" + (transactions.size() + 1),AccountId, "TRANSFER", amount, 
new Date().toString(), "Received: " + remarks)); 
} 
